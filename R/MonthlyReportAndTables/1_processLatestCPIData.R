...
#### Fortnight Prices and Weights####

# Read in file for Prices
forthnightFolder <- file.path(repository, "data", "secure", "Forthnight Collections")

firstForthnight_File <- file.path(forthnightFolder, "CPI_ForthnightPrice_PVila_DataEntryForm_F1.csv")
portVilaF1PriceStats <- read.csv(firstForthnight_File)

secondForthnightPrice_file <- file.path(forthnightFolder, "CPI_ForthnightPrice_PVila_DataEntryForm_F2.csv")
portVilaF2Price_stats <- read.csv (secondForthnightPrice_file)

thirdForthnightPrice_file <- file.path(forthnightFolder, "CPI_ForthnightPrice_PVila_DataEntryForm_F3.csv")
portVilaF3Price_stats <- read.csv (thirdForthnightPrice_file)

fourthForthnightPrice_file <- file.path(forthnightFolder, "CPI_ForthnightPrice_PVila_DataEntryForm_F4.csv")
portVilaF4Price_stats <- read.csv (fourthForthnightPrice_file)

fifthForthnightPrice_file <- file.path(forthnightFolder, "CPI_ForthnightPrice_PVila_DataEntryForm_F5.csv")
portVilaF5Price_stats <- read.csv (fifthForthnightPrice_file)

sixthForthnightPrice_file <- file.path(forthnightFolder, "CPI_ForthnightPrice_PVila_DataEntryForm_F1.csv")
portVilaF6Price_stats <- read.csv (sixthForthnightPrice_file)

# Read CPI Forthnight Weights Spreadsheet

firstForthnightWeights_file <- file.path(forthnightFolder, "CPI_ForthnightWeight_PVila_DataEntryForm_F1.csv")
portVilaF1Weights_stats <- read.csv (firstForthnightWeights_file)

secondForthnightWeights_file <- file.path(forthnightFolder, "CPI_ForthnightWeight_PVila_DataEntryForm_F2.csv")
portVilaF2Weights_stats <- read.csv (secondForthnightWeights_file)

thirdForthnightWeights_file <- file.path(forthnightFolder, "CPI_ForthnightWeight_PVila_DataEntryForm_F3.csv")
portVilaF3Weights_stats <- read.csv (thirdForthnightWeights_file)

fourthForthnightWeights_file <- file.path(forthnightFolder, "CPI_ForthnightWeight_PVila_DataEntryForm_F4.csv")
portVilaF4Weights_stats <- read.csv (fourthForthnightWeights_file)

fifthForthnightWeights_file <- file.path(forthnightFolder, "CPI_ForthnightWeight_PVila_DataEntryForm_F5.csv")
portVilaF5Weights_stats <- read.csv (fifthForthnightWeights_file)

sixthForthnightWeights_file <- file.path(forthnightFolder, "CPI_ForthnightWeight_PVila_DataEntryForm_F6.csv")
...
#### 
#### Aggregate of Prices for the Forthnight ####
...
#### Dividing the Aggregate Price over Aggregate Weights for Quater ####
...
#Creating a data frame of the Masterspreadsheet and await to be merged.

#Tomorrow's work
#1. Create a vector cointaing all that is in the specification column.
#2. Filter the Data (cpi_stats) with this vector
#3. Input the calculations from the (Quarter(PriceperKilo)) into the filtered masterspreadsheet.

tarFilter <- c("Apples, loose, per Kg", "Banana, cooking, bunch, per Kg", "Banana, eating, bunch, per Kg", "Cabbage, bowl, per Kg", "Cabbage, Chinese, per Kg",
               "Cabbage, Island, per Kg", "Carrots, imported, loose, per Kg", "Carrots, loose, per Kg", "Chilli, loose, per Kg", "Coconut, dried, per Kg?", "Corn, loose, per Kg",
               "Cucumber, loose, per Kg", "DVD player, name brand, single disc", "Dwarf beans, loose, per Kg", "English loaf, 30 cm, unsliced", "French loaf, 30 cm, unsliced", "Garlic, loose, per Kg", 
               "Ginger, loose, per Kg", "Grapes, bunch, per Kg", "Green coconut, piece, per Kg", "Island taro, basket, per Kg", "Kiwi fruit, loose, per Kg", "Kumala Per Kilo", "Laplap (Yam, banana), standard?, per Kg?", 
               "Lettuce, imported, per Kg", "Lettuce, per Kg", "Limes, loose, per Kg", "Long beans, loose, per Kg", "Mandarines, imported, loose, per Kg", "Mandarines, local, loose, per Kg", "Manioc (kassava), basket, per Kg", 
               "Onions, loose, per Kg", "Oranges, loose, per Kg", "Pamplemous, loose, per Kg", "Parsley, bunch, per Kg","Passion fruits, fresh, local","Pawpaws, loose, per Kg", "Pears, loose, per Kg", "Pineapples, loose, per Kg",
               "Potatoes, red, loose, per Kg", "Potatoes, white, loose, per Kg","Pumpkin top, loose, per Kg","Pumpkin, per Kg","Radish, pkt, per Kg","Rock melon, loose, per Kg","Snake Beans, loose, per Kg","Sugurcane, unit? Weight? Per Kg?",
               "Sweet potato (kumala), basket, per Kg","Taro (Fiji) Per Kilo","Tomato, imported, loose, per Kg","Tomato, loose, per Kg","Vanilla, loose, per Kg","Water kress, per Kg","Yam, basket, per Kg","Yam, Per Kilo")

filter(cpi_stats, specifications %in% target)